...
pub fn print_compare_stats(s1: &LayoutStats, s2: &LayoutStats, data: &AnalyzerData) {
    ...
    println!(
        ...
        format!("{:.3}", s1.fspeed),
        s2.fspeed,
        format!("{:.3}", s1.stretches),
        s2.stretches,
        format!("{:.3}%", s1.scissors),
        s2.scissors,
        format!("{:.3}%", s1.lsbs),
        s2.lsbs,
        format!("{:.3}%", s1.pinky_ring),
        s2.pinky_ring,
        format!("{:.2}%", ts1.inrolls),
        ts2.inrolls,
        format!("{:.2}%", ts1.outrolls),
        ts2.outrolls,
        format!("{:.2}%", (ts1.inrolls + ts1.outrolls)),
        (ts2.inrolls + ts2.outrolls),
        format!("{:.3}%", ts1.onehands),
        ts2.onehands,
        format!("{:.2}%", ts1.alternates),
        ts2.alternates,
        format!("{:.2}%", ts1.alternates_sfs),
        ts2.alternates_sfs,
        format!("{:.2}%", (ts1.alternates + ts1.alternates_sfs)),
        (ts2.alternates + ts2.alternates_sfs),
        format!("{:.3}%", ts1.redirects),
        ts2.redirects,
        format!("{:.3}%", ts1.redirects_sfs),
        ts2.redirects_sfs,
        format!("{:.3}%", ts1.bad_redirects),
        ts2.bad_redirects,
        format!("{:.3}%", ts1.bad_redirects_sfs),
        ts2.bad_redirects_sfs,
        format!(
            "{:.3}%",
            (ts1.redirects + ts1.redirects_sfs + ts1.bad_redirects + ts1.bad_redirects_sfs)
        ),
        (ts2.redirects + ts2.redirects_sfs + ts2.bad_redirects + ts2.bad_redirects_sfs),
        format!("{:.3}%", ts1.bad_sfbs),
        ts2.bad_sfbs,
        format!("{:.3}%", ts1.sfts),
        ts2.sfts,
