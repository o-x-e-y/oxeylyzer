...
export default function AnalyzeView(props: Props) {
    ...
    async function analyzeState(baseName: string, keys: string, disabledIdxs: number[]): Promise<Layout> {
        const isOriginal = disabledIdxs.length === 0 && keys === (baseline()?.keys ?? "");
        return isOriginal
            ? analyzeLayout(baseName)
            : analyzeCustom(baseName, keys, disabledIdxs);
    }
    ...
    function handleReset() {
        ...
        analyzeLayout(baseName).then((fresh) =>
            applyIfCurrent(s, () => setLayout(fresh))
        );
    }
    ...
    return (
        <div class="flex-1 min-h-0 flex flex-col gap-3 overflow-hidden">
            ...
            <div class="flex gap-4 flex-1 min-h-0">
                {/* Col 1 — keyboard ───────────────────────────────────── */}
                <div class="shrink-0 flex flex-col gap-2 w-80">
                    <Show when={layout()}>
                        {(l) => (
                            <KeyboardDisplay
                                keys={l().keys}
